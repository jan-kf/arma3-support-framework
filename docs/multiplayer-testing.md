...
SteamCMD's Linux dedicated-server payload omits the base playable-character PBO. The integration mission uses one `B_Soldier_F` player slot, so the controller bind-mounts only the authenticated client's licensed `Addons/characters_f.pbo` into the server's `addons/` directory as read-only. Without it, the server reports that `a3_characters_f` has been deleted and cannot create the mission's player entity.
