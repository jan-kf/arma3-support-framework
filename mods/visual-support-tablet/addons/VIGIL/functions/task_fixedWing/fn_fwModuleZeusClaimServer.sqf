...
private _operations = localNamespace getVariable ["YSF_FW_ZEUS_OPERATIONS", createHashMap];
...
if (_operationId isEqualType "" && {_operationId isNotEqualTo ""} && {_prior isNotEqualTo []}) exitWith {
    private _audit = localNamespace getVariable ["YSF_FW_ZEUS_CLAIM_AUDIT", []];
    _audit pushBack [_operationId, _logicId, _curatorId, _requesterOwner, false, "duplicate", diag_tickTime, []];
    localNamespace setVariable ["YSF_FW_ZEUS_CLAIM_AUDIT", _audit];
    [_operationId, _logicId, false, "duplicate", ""] remoteExecCall ["YSF_fnc_fwModuleZeusResult", _requesterOwner];
    false
};
if (_operationId isEqualType "" && {_operationId isNotEqualTo ""}) then {
    ...
    while {(count _operations) > 127} do {
        ...
        {private _at = (_operations getOrDefault [_x, []]) param [4, diag_tickTime]; if (_at < _oldestAt) then {_oldestAt = _at; _oldestKey = _x;};} forEach keys _operations;
        if (_oldestKey isEqualTo "") exitWith {};
        _operations deleteAt _oldestKey;
    };
    localNamespace setVariable ["YSF_FW_ZEUS_OPERATIONS", _operations];
};
[_logic, _curator, _operationId, _requesterOwner, _logicId, _curatorId] spawn {
    ...
    private _predicates = [
        ...
        ["logic_class", !isNull _logic && {typeOf _logic isEqualTo "YSF_FixedWing_Zeus_Add_Module"}],
    ...
    localNamespace setVariable ["YSF_FW_ZEUS_CLAIM_AUDIT", _audit];
    if (!_valid && {_requesterOwner > 2}) then {[_operationId, _logicId, false, _reason, ""] remoteExecCall ["YSF_fnc_fwModuleZeusResult", _requesterOwner];};
};
true
