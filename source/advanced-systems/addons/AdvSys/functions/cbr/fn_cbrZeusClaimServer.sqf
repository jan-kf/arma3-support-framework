...
private _operations = localNamespace getVariable ["YAS_CBR_ZEUS_OPERATIONS", createHashMap];
private _now = diag_tickTime;
{
	private _row = _operations getOrDefault [_x, []];
	if (_now - (_row param [4, _now]) > 300) then {_operations deleteAt _x;};
...
if (_operationId isEqualType "" && {_operationId isNotEqualTo ""} && {_prior isNotEqualTo []}) exitWith {
	private _audit = localNamespace getVariable ["YAS_CBR_ZEUS_CLAIM_AUDIT", []];
	_audit pushBack [_operationId, _logicId, _curatorId, _requesterOwner, false, "duplicate", diag_tickTime, []];
	localNamespace setVariable ["YAS_CBR_ZEUS_CLAIM_AUDIT", _audit];
	[_operationId, _logicId, false, "duplicate", missionNamespace getVariable ["YOSHI_CBR_ENABLED", false]] remoteExecCall ["YAS_fnc_cbrZeusToggleResult", _requesterOwner];
	false
};
if (_operationId isEqualType "" && {_operationId isNotEqualTo ""}) then {
	...
	while {(count _operations) > 127} do {
		...
		{
			private _at = (_operations getOrDefault [_x, []]) param [4, _now];
			if (_at < _oldestAt) then {_oldestAt = _at; _oldestKey = _x;};
		} forEach keys _operations;
		if (_oldestKey isEqualTo "") exitWith {};
		_operations deleteAt _oldestKey;
	};
	localNamespace setVariable ["YAS_CBR_ZEUS_OPERATIONS", _operations];
};
[_logic, _curator, _operationId, _requesterOwner, _logicId, _curatorId] spawn {
	...
	private _predicates = [
		...
		["logic_class", !isNull _logic && {typeOf _logic isEqualTo "YAS_CBR_Zeus_Toggle_Module"}],
	...
	if (_valid) then {
		...
		_claims set [_logicId, [_operationId, _curatorId, _requesterOwner, "claimed"]];
		localNamespace setVariable ["YAS_CBR_ZEUS_CLAIMS", _claims];
	};
	_operations = localNamespace getVariable ["YAS_CBR_ZEUS_OPERATIONS", createHashMap];
	...
	if (!_valid && {_requesterOwner > 2}) then {
		[_operationId, _logicId, false, _reason, missionNamespace getVariable ["YOSHI_CBR_ENABLED", false]] remoteExecCall ["YAS_fnc_cbrZeusToggleResult", _requesterOwner];
	};
};
true
