...
EVIDENCE_CONTRACT = {
    "scenario": {
        ...
        "definition": {
            ...
            "applicability": "Arma 3 dedicated multiplayer with one authenticated client and server-owned supply/carrier fixtures",
            "participants": {
    ...
    "unresolved": [
        "Client-owned cargo/carriers, ownership migration, client-B/JIP, full-carrier breadth, unload UX, and automatic physical-contact attachment remain outside this proof."
    ],
}
...
TRIBUNAL_SCENARIO = Scenario(
    ...
    review=ScenarioReview(
        ...
        behavior_contract="A nearby authenticated player can invoke the exact registered action to load one eligible supply into one eligible server-owned carrier; success requires native command acceptance plus exact replicated membership, while replay, range, class, and collateral controls fail closed.",
        outcome="REFINE BEFORE PERMANENT COVERAGE",
        rationale="The former action mutated cargo locally, discarded the command result, and retained a literal-true stale condition. The refinement adds server authentication, activation-time validation, exact result identity, replay protection, independent membership evidence, negatives, and cleanup.",
        dependencies=("ACE 3.21 registered interaction tree", "native vehicle-in-vehicle cargo", "one authenticated client"),
        evidence_types=frozenset({"ace-action-data", "exact-identity", "authoritative-state", "native-command-result", "replication", "negative-control", "cleanup"}),
        locality_requirements="The server owns all fixture objects and cargo mutation; client-a owns action resolution and authenticated requests. Client-owned objects, migration, and client-B/JIP are unclaimed.",
    ),
