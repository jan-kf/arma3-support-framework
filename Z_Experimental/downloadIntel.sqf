			...
			while { _progress < 100 && !_downloadComplete } do {
				...
				if (!_interrupt) then {
					...
					if ((random 100 < 10)) then {
						...
						[_error] remoteExec ["systemChat"];
					};
				} else {
					["Waiting user input to continue..."] remoteExec ["systemChat"];
				};
...
private _kickAction = ["KickDownload", "Kick it", "",  
	{
		...
		["User override accepted, proceeding..."] remoteExec ["systemChat"];
	},
	{ 
		params ["_target", "_caller", "_params"];
		_shouldKick = _target getVariable ["shouldKick", false];
...
[_object, 0, ["ACE_MainActions"], _downloadAction] call ace_interact_menu_fnc_addActionToObject;
[_object, 0, ["ACE_MainActions"], _kickAction] call ace_interact_menu_fnc_addActionToObject;