...
private _closestDistance = 10;
{
    private _markerPos = getMarkerPos _x;
    private _distance = _pos distance _markerPos;
    
    if (_distance < _closestDistance) then {
        _closestDistance = _distance;
        _closestMarker = markerText _x;
    };
} forEach allMapMarkers;

if (_closestMarker != "") then {
    // this is a marker
    [0, "marker", _closestMarker]
    ...
    if (!(_nearestLocations isEqualTo [])) then {
        ...
        private _direction = [_locationPos, _pos] call BIS_fnc_dirTo;
