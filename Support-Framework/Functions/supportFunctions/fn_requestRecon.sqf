...
if ((isTouchingGround _vic) && (speed _vic < 1)) then {
	...
	sleep 3;
	[driver _vic, format ["Cleared for reconnaissance to %1, %2 out.", _gridRef, groupId group _vic]] call SupportFramework_fnc_sideChatter;
}else{
	...
	[_baseCallsign, format [_base_to_vic_msg, groupId group _vic, _gridRef]] call SupportFramework_fnc_sideChatter;
	sleep 3;
	[driver _vic, format ["Cleared for reconnaissance at %1, %2 out.", _gridRef, groupId group _vic]] call SupportFramework_fnc_sideChatter;
};
