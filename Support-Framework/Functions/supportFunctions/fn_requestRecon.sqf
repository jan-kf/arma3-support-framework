...
private _currentPos = getPosASL _vic;
...
if ([_vic] call YOSHI_fnc_hasLanded) then {
	...
	sleep 3;
	[driver _vic, format ["Cleared for reconnaissance to %1, %2 out.", _gridRef, groupId group _vic]] call YOSHI_fnc_sendSideText;
}else{
	...
	[_baseCallsign, format [_base_to_vic_msg, groupId group _vic, _gridRef]] call YOSHI_fnc_sendSideText;
	sleep 3;
	[driver _vic, format ["Cleared for reconnaissance at %1, %2 out.", _gridRef, groupId group _vic]] call YOSHI_fnc_sendSideText;
};
